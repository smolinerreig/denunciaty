...
class ControllerConsole {
	 ...
	 *
	 * @param array $params
	 *        	parametros nombrados de la consola
	...
	public function create($params, $controller) {
		...
		if (! is_file ( $file ) || Console::input ( "El controlador existe, ¿desea sobrescribirlo? (s/n): ", array (
				's',
				'n' 
		) ) == 's') {
	...
	public function delete($params, $controller) {
		...
		if (is_dir ( $views_dir ) && Console::input ( '¿Desea eliminar el directorio de vistas? (s/n): ', array (
				's',
				'n' 
		) ) == 's') {
