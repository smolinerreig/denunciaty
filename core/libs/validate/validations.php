...
class Validations {
	...
	public static function alphanum($string) {
		return self::pattern ( $string, array (
				'regexp' => self::IS_ALPHANUM 
		) );
	}
	 ...
	 * 
	 * @param string $value
	 *        	fecha a validar acorde al formato indicado
	 * @param array $param
	 *        	como en DateTime
	 * @return boolean
	 */
	...
	public static function pattern($check, $param) {
		...
		return empty ( $check ) || FALSE !== filter_var ( $check, FILTER_VALIDATE_REGEXP, array (
				'options' => array (
						'regexp' => $regex 
				) 
		) );
