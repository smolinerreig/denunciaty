...
abstract class Upload {
	 ...
	 *
	 * @param string $name
	 *        	nombre de archivo por método POST
	 */
	 ...
	 *        	lista de tipos de archivos permitidos (mime) si es string separado por |
	 */
	 ...
	 *
	 * @param array|string $value
	 *        	lista de extensiones para archivos, si es string separado por |
	 */
	 ...
	 *
	 * @param string $name
	 *        	nombre con el que se guardo el archivo
	 * @return boolean|null
	 */
	 ...
	 *
	 * @param string $name
	 *        	nombre de archivo recibido por POST
	...
	protected function _allowScripts() {
		return $this->_cond ( ! $this->_allowScripts && preg_match ( '/\.(php|phtml|php3|php4|js|shtml|pl|py|rb|rhtml)$/i', $_FILES [$this->_name] ['name'] ), 'no esta permitido subir scripts ejecutables' );
	}
	...
	protected function _types() {
		return $this->_cond ( ! in_array ( $_FILES [$this->_name] ['type'], $this->_types ), 'el tipo de archivo no es válido' );
	}
	protected function _extensions() {
		return $this->_cond ( ! preg_match ( '/\.(' . implode ( '|', $this->_extensions ) . ')$/i', $_FILES [$this->_name] ['name'] ), 'la extensión del archivo no es válida' );
	}
	protected function _maxSize() {
		return $this->_cond ( $_FILES [$this->_name] ['size'] > $this->_toBytes ( $this->_maxSize ), "no se admiten archivos superiores a $this->_maxSize b" );
	}
	protected function _minSize() {
		return $this->_cond ( $_FILES [$this->_name] ['size'] < $this->_toBytes ( $this->_minSize ), "Error: no se admiten archivos inferiores a $this->_minSize b" );
	}
}
