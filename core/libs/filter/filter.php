...
class Filter {
	...
	public static function get_array($array, $filter, $options = array()) {
		...
		foreach ( $array as $k => $v ) {
			$args [0] = $v;
			$array [$k] = call_user_func_array ( array (
					'self',
					'get' 
			), $args );
		}
	 ...
	 *        	datos a filtrar.
	 * @param array $fields
	 *        	arreglo donde los indices son los campos a devolver
	 ...
	 *        	filtros que se aplicaran a todos los elementos.
	...
	public static function data(array $data, array $fields, $filterAll = '') {
		...
		foreach ( $fields as $index => $filters ) {
			if (is_numeric ( $index ) && array_key_exists ( $filters, $data )) {
				...
				$filtered [$index] = call_user_func_array ( array (
						'self',
						'get' 
				), $filters );
	...
	public static function get_object($object, $filter, $options = array()) {
		...
		foreach ( $object as $k => $v ) {
			$args [0] = $v;
			$object->$k = call_user_func_array ( array (
					'self',
					'get' 
			), $args );
		}
