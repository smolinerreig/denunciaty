...
class Rest {
	 ...
	 */
	private static $_inputFormat = array (
			'json',
			'plain',
	 ...
	 *
	 * @param string $accept
	 *        	Cada uno de los tipos separados por coma ','
	 */
	 ...
	 *        	controlador que se convertira en un servicio REST
	 */
	static public function init(Controller $controller) {
		 ...
		 */
		self::$_iFormat = str_replace ( array (
				'text/',
				'application/' 
		), '', $content );
		...
		while ( self::$_oFormat = array_shift ( $format ) ) {
			self::$_oFormat = str_replace ( array (
					'text/',
					'application/' 
			), '', self::$_oFormat );
