...
class KumbiaActiveRecord {
	...
	public function convert_params_to_sql($what = '') {
		...
		if (is_array ( $what )) {
			...
			if (count ( $limit_args ) > 1) {
				$select = call_user_func_array ( array (
						$this,
						'limit' 
				), $limit_args );
			}
	 ...
	 * 
	 * @param string $what        	
	 * @return array Soporta parametros iguales a find
	 *        
	 */
	 ...
	 *
	 * @param string $form
	 *        	form name for request, equivalent to $_REQUEST[$form]
	 * @return boolean success
	 * @deprecated No es seguro
	 */
	...
	public function save($values = null) {
		...
		foreach ( $this->_validates ['format_of'] as $f => $opt ) {
			if (isset ( $this->$f ) && ! is_null ( $this->$f ) && $this->$f != '') {
				if (! filter_var ( $this->$f, FILTER_VALIDATE_REGEXP, array (
						"options" => array (
								"regexp" => $opt ['pattern'] 
						) 
				) )) {
	 ...
	 *
	 * @param
	 *        	s string $field campo a validar
	 * @param int $max
	 *        	valor maximo
	 * @param int $min
	 *        	valor minimo
	 ...
	 *        	expresion regular para preg_match
	 *        	
	 *        	message: mensaje a mostrar
	 *        	field: nombre del campo a mostrar en el mensaje
	 */
	 ...
	 *
	 * @param string $model        	
	 * @return ActiveRecord @throw KumbiaException
	 *        
	 */
