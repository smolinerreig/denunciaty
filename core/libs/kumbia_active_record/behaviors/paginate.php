...
class Paginator {
	 ...
	 *
	 * @example $page = paginate($array, 'per_page: 5', "page: $page_num"); <br>
	...
	public static function paginate($model) {
		...
		if (is_array ( $model )) {
			...
			$page->items = call_user_func_array ( array (
					$model,
					'find' 
			), $find_args );
		}
	 ...
	 *        	count: Total de registros
	 *        	
	 * @example $page = paginate_by_sql('usuario', 'SELECT * FROM usuario' , 'per_page: 5', "page: $page_num");
	 *         
	 * @return stdClass
	 *
	 */
